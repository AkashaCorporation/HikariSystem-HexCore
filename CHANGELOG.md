...
#### Headless job-file discovery — implicit *Run Job* kept root-only (investigated, no behavior change)

> A "No .hexcore_job.json file was found." report on a no-arg *Run Job* was traced to a stale compiled `out/` (the IDE runs `out/`, which had drifted ~6.5 min behind `src/` after interleaved `tsc` runs) plus the fact that the workspace had **12 named job files in a subfolder** and no canonical root job. A recursive auto-discovery was prototyped and **deliberately reverted**: with many candidate jobs, any auto-pick is a guess — it would silently run an arbitrary job (alphabetically first) and report it "completed", which reads as a false positive. Per "a fake feature is worse than none", implicit discovery stays **workspace-root-only** (unchanged from 3.8.1) and emits an honest "not found" rather than running the wrong job.

- **No behavior change for existing users.** A canonical `.hexcore_job.json` (or a named job) at the workspace root resolves exactly as before. To run a job that lives in a subfolder, pass its path or right-click the file (the explicit-path branches are unaffected), or open the workspace at the folder containing it.
- **Subfolder jobs still auto-run via the recursive `FileSystemWatcher`** on save — that path was always recursive and is untouched.
- In-code comments now document *why* implicit discovery is root-only, so the "make it recursive" change is not re-attempted without handling the multi-candidate ambiguity.
