...
- Windows distribution remains intentionally portable and manually updated. Builds without an update endpoint no longer initialize Electron's `appUpdate` cache path or emit the resulting nonfatal startup exception.
- The Windows installer workflow now treats Helix as a required engine instead of a best-effort prebuild and fails when its `.node` is absent.
- Every packaged ZIP now includes `hexcore-engine-manifest.json`, binding the product commit to the version, release tag, byte size, and SHA-256 of every bundled native `.node`, runtime DLL/executable, and Remill semantics bitcode artifact. Rellic remains listed explicitly as disabled compatibility scope.
...
- Revenant `0.4.0` now ships its self-contained ILSpy 10 backend from the standalone `v0.4.0` release. The installer downloads the platform asset into `bin/<platform>/`, and an explicit `.vscodeignore` keeps that prebuild in the extension package even though `bin/` remains ignored by Git.
...
> - **Unbounded recursion:** `readExpr` / `readStmt` / `readSwitchCase` / `readTypeStr` recurse once per AST nesting level with no depth bound, and child offsets are resolved via `__indirect` with no validation -- a deeply-nested (or cyclic-offset) buffer blows the JS stack (`RangeError`), and with no try/catch around `hydrateHAST` the whole scan aborts.
> - **Unclamped vector count:** `readVec` returned the raw `__vector_len` (an attacker int32 up to 2^31) without comparing it to the buffer length, so every caller's element loop could run billions of times reading garbage -- an OOM / hang.
> - **`parseInt` miscompile:** the If-statement then/else split used `parseInt(text, 10)` with no NaN/range guard (the sibling For-case already had `|| 0`), so a non-numeric/negative count silently swapped or lost the then vs else bodies.

- **Fix:** (1) `readVec` rejects (returns null) any vector whose `[start, start + len*4)` range exceeds `bb.capacity()` -- one chokepoint that bounds all 8 callers; (2) the per-function loop in `hydrateHAST` wraps `readFunction` in try/catch so a pathological function is skipped and the rest of the module still hydrates (the recursion overflow is contained here -- a function that survives is <= the hydrator's ~4095 limit, well under the matcher's ~9631); (3) the If-case clamps `thenCount` to `[0, children.length]` and falls back to `children.length` on NaN, mirroring the For-case.
- **Validated** before/after on a REAL decompiled HAST buffer (Helix `decompileIr().astBuffer`): AFTER and BEFORE (guards stripped) hydrate IDENTICALLY (1 function, 2 body stmts, matcher fires CIfStmt x2 / CBinaryExpr x6) -- the fixes are transparent for valid input; a truncated buffer (every vector now exceeds capacity) hydrates to 0 functions gracefully in 0 ms AFTER (no throw, no hang). hexcore-hql `0.1.0 -> 0.1.1`.
...
> - **Unbounded stdout:** the ARM64 `write(1|2,...)` syscall appended up to ~64 KiB per call to `_arm64StdoutBuffer` with no cumulative cap -- a crafted write-loop reaches multi-GiB within a single instruction-budgeted continue.
> - **Unbounded trace:** `TraceManager.record` pushed every intercepted API/syscall with no cap; it survives across continue calls within a session, so a hooked-call loop grows it unboundedly.
> - **Stale-after-dispose:** `disposeEmulation()` nulled the engines but did NOT clear the session buffers (stdout / syscall-log / side-channel / trace), so the getters returned a torn-down prior session's data as if current.

- **Fix:** (1) the write syscall caps `_arm64StdoutBuffer` at 8 MiB (one-time truncation marker); (2) `TraceManager.record` caps retained entries at `HEXCORE_TRACE_MAX_ENTRIES` (default 200000, env-overridable, mirroring the existing `HEXCORE_SC_MAX_ADDRS`), dropping the overflow while keeping the trace's beginning and surfacing a `dropped` count in the JSON export (listeners still fire for real-time streaming); (3) the per-session resets were extracted into a `resetSessionState()` now called at the END of `disposeEmulation()` as well as the start of `startEmulation()`.
- **Validated** before/after on the compiled `out/` (BEFORE modules = guard text stripped from the compiled JS; a mock emulator drives the write syscall so the harness never actually allocates): 200 x 64 KiB writes grow stdout to 12.5 MiB BEFORE and plateau at ~8 MiB AFTER; `record` of 25 entries at a cap of 10 retains 10 + reports `dropped=15` (was 25, no count); a bare `disposeEmulation()` leaves stale stdout/trace BEFORE and returns empty AFTER. Behaviour-preserving for legitimate runs (the caps sit far above any real trace/stdout). Debugger `2.1.3 -> 2.1.4`.

### Emulation - clamp the PE/ELF loader memory-map + TLS allocation sizes (untrusted-binary OOM; debugger 2.1.3)

> A 5-dimension adversarial parser-bounds audit of the hexcore-debugger PE/ELF emulator loaders (`peLoader.ts` + `elfLoader.ts`, which parse an UNTRUSTED binary and map it into Unicorn memory) confirmed 10 of 22 candidates -- 12 were refuted after default-refuted verification (the BigInt precision-loss, the RVA dead-code, and the loops/writes that Unicorn already rejects with UC_ERR). The MEDIUM/HIGH confirmed findings are one class: an attacker-controlled header size driving a multi-GB native allocation with no bound.
> - **PE SizeOfImage** -> `alignedImageSize` -> `mapMemoryRaw(imageBase, alignedImageSize, RWX)` (medium); **ELF PT_LOAD p_memsz** -> `alignedSize` -> `mapMemoryRaw` (high). A tiny file declaring a ~4 GiB image/segment forced a giant native `uc.memMap` -- the inline comments at the sibling map sites already call an OOM there "unrecoverable" (a process-abort of the analyst's session).
> - **PE TLS SizeOfZeroFill** -> `tlsDataBytes` drove BOTH a `mapMemoryRaw` AND a host `Buffer.alloc(tlsDataBytes)` (medium) -- a ~4 GiB map plus a ~4 GiB Node Buffer.

- **Fix:** (1) a single `assertMapSize` chokepoint at the top of `mapMemoryRaw` (mirroring the `assertReadSize` guard already in that file) rejects a non-integer / `<= 0` / `> 2 GiB` size BEFORE the native `uc.memMap`, so the multi-GB map can never be committed -- one guard defends every map caller (image, segment, TLS, and any future one). The 2 GiB ceiling sits far above every legitimate map (largest fixed region is 8 MiB; a real in-memory image is well under 1 GiB). (2) `setupStaticTls` clamps `tlsDataBytes` to `TLS_STORAGE_SIZE` -- the fixed TLS region the rest of the loader already assumes (the template read was already bounded to it) -- which bounds the host `Buffer.alloc` the chokepoint cannot see.
- **Validated** before/after on the compiled `out/` (BEFORE module = guard text stripped from the compiled JS; a mock emulator records the size handed to the map sink so the harness itself never allocates): a `0xFFFFFFFF` map reaches a ~4 GiB native `memMap` BEFORE and is blocked pre-native AFTER (an 8 MiB legitimate map is unaffected); the TLS path maps 64 MiB BEFORE and is clamped to `TLS_STORAGE_SIZE` (64 KiB) AFTER. Behaviour-preserving for legitimate binaries. Debugger `2.1.2 -> 2.1.3`. The audit's 7 LOW findings (min-length guards before fixed-offset header reads, section write-destination validation, and an uncapped / zero-stride phNum/shNum loop -- all of which currently fail closed via a thrown RangeError / Unicorn UC_ERR) are filed as #50.
...
- **Fix:** a one-shot `wakeup` resolver makes the park wakeable (`Promise.race([delay(100), wakeupPromise])`); `scheduleProcessLoop` signals it when it would otherwise no-op (the loop is already running). The 100 ms timer remains as a fail-safe re-poll, so the dispatch decision is unchanged -- this is purely a latency optimization. Both park sites (all-busy and no-dispatchable-session-job) are wakeable.
- **Validated** before/after on the compiled `out/`: dispatch latency after a worker frees mid-park drops from ~40 ms (the poll timer) to ~2 ms (the wakeup); the full `jobQueueManager` suite is 13 passing (added a deterministic latency regression test). Disassembler `1.4.11 -> 1.4.12`. Item 3 of #45 (slot-hold-on-cancel) stays open and was re-scoped: eager release is NOT safe -- freeing the slot while the cancelled executor is still running (until its next step boundary) would let the slot be reused concurrently (transient over-subscription), so holding the slot until the executor actually settles is the correct concurrency behavior; the real improvement is finer-grained abort polling in the runner, not eager release in the queue.
...
- **Fix (`hexcore-disassembler/jobQueueManager.ts`):** when no genuinely UNBOUND free worker exists, a session job with no binding yet now WAITS (returns `undefined`) instead of falling back to a bound-but-free worker. The slot stays reserved for its own queued sibling and becomes a generic free worker only once its session fully drains -- no permanent starvation beyond normal pool contention; stateless jobs (handled earlier in the function) are untouched.
- **Validated** engine-direct before/after on the compiled `out/` with a crafted scenario (one worker bound-but-free to session S1 with a queued S1 sibling, the other worker busy, a new higher-priority session S3 dispatched): before, S3 stole S1's worker; after, S3 waits and S1's queued sibling reclaims its own worker (affinity preserved). Added a deterministic regression test (`#45 dispatch refinements`) -- the full `jobQueueManager` suite is 12 passing. Disassembler `1.4.10 -> 1.4.11`. Items 2 (worker-freed-during-park ~95ms wakeup latency) and 3 (slot-hold-until-step-boundary on cancel) of #45 remain open.

### Remill - lift PAST an unsupported AArch64 SIMD instruction instead of truncating the function (FIX-105, the AArch64 CFG-truncation gap)

> Branch-heavy AArch64 functions decompiled to ~26% of their body, the tail stubbed as `common.ret` -- the long-standing "AArch64 CFG truncation" gap. Root cause (NOT a decode failure, NOT Helix): an AArch64 SIMD instruction Remill RECOGNISES but ships no semantic for -- e.g. `STP Qt, Qt2, [Xn]` (128-bit NEON store-pair, encoding `0xad0003e0`) -- decodes fine, so FIX-024's decode-failure NoOp path never fires. `LiftIntoBlock` emits a `HandleUnsupported` side-effect call and returns `kLiftedUnsupportedInstruction` (not `kLiftedInstruction`). The Phase-3 lift loop treated every non-`kLiftedInstruction` status as fatal and `break`-ed, discarding every instruction after the SIMD op; and when the unsupported instruction was the FIRST in the function, the `totalOffset == 0` guard returned an EMPTY lift -- the whole function vanished.
>
> Confirmed by ground-truth engine-direct lifts (NOT source reasoning): `mov w0,#1; stp q0,q0,[sp]; ret` consumed only 4 of 12 bytes (stopped at the SIMD store); `stp q0,q0,[sp]; cbz w0,#8; add; ret` produced a COMPLETELY EMPTY IR (0/16 bytes). A prior multi-agent fan-out's top "confirmed" finding (a conditional-branch helper swap) was empirically a no-op -- the AArch64 `cbz` already lifts correctly with a `br i1`; the sole truncation point is the unsupported-SIMD `break`.

- **Fix (`remill_wrapper.cpp`, Phase-3 lift):** on `kLiftedUnsupportedInstruction` (or a FIX-024 `kCategoryNoOp` decode-failure stub), advance the byte counter and CONTINUE to the next instruction instead of `break`-ing. Remill has already emitted the `HandleUnsupported` call into the block, so it stays well-formed; the rest of the function lifts and its branches / blocks are recovered. A genuine hard error (invalid instruction / internal lifter error) on a real instruction still aborts exactly as before.
- **Validated before/after on the rebuilt `build/Release/hexcore_remill.node`:** SIMD-in-the-middle 4/12 -> 12/12 bytes (the post-store `ret` now lifts); SIMD-first-with-branch 0/16 -> 16/16 bytes WITH the `cbz` recovered (`br i1`, 1 -> 3 blocks); a two-`STP-Q` body 4/20 -> 20/20. **Zero x86 regression:** the real `crackme.deflat2` x86 lift is byte-identical (978 `br`, unchanged) and every non-SIMD AArch64 / x86 control IR is SHA-identical before/after. The fix is status-gated (only reachable on a failed lift), not arch-gated, so x86 is provably untouched.
...
### BTF loader: spec-correct INT encoding/bits + clamp oversized ARRAY size (autonomous audit loop, #47)

> Follow-up to the BTF parser-bounds hardening, closing 2 of the 3 items in #47. (1) `parseTypes` extracted the `btf_int` fields with inverted masks -- `encoding = VAL & 0xFF` actually read nr_bits and `bits = (VAL >> 16) & 0xFFFF` read garbage; now spec-correct (`encoding = (VAL >> 24) & 0x0F`, `bits = VAL & 0xFF`). (2) `resolveTypeSize` for an ARRAY returned `elemSize * nelems` with `nelems` an attacker-controlled u32, yielding an absurd size in the exported struct-info JSON; now capped to a 1 GiB ceiling. Validated before/after on the compiled artifact + 2 new unit tests (5 passing). Disassembler `1.4.9 -> 1.4.10`. (Item 3 -- member-offset kflag propagation, >2 MB structs only -- remains open in #47.) Also audited clean this round: `capstoneWrapper.ts` (decode classification + bounds-safe fallback prologue scan) and `llvmMcWrapper.ts` (patch sizing + NOP-alignment guards) -- no fix needed.
...
>
> - **Unbounded `Buffer.alloc` (defense-in-depth):** the section-header-string-table and `.BTF` section sizes are read straight from the untrusted ELF (up to 2^53) and passed to `Buffer.alloc` with no clamp; the `.BTF` section size also flowed into `fs.readSync`'s length, where a 10 GiB value overflows the 32-bit length arg to a negative number and throws. Both the alloc and the read length are now clamped to the real file size (`Math.min(size, fs.fstatSync(fd).size)`) -- a section can never exceed the file that contains it.
> - **OOB read on a corrupt/oversized header (data-loss):** the type-section and string-table bounds used header-derived ends (`hdrLen + typeOff + typeLen`, all attacker-controlled), not the actual buffer length, so an inflated `typeLen`/`strLen` read past the buffer and threw `RangeError` -- which `loadBtfFromFile` swallowed to `null`, dropping ALL of a partially-corrupt module's debug info. `typeEnd`/`strEnd` are now clamped to `data.length`, so the valid prefix parses instead of losing everything.
> - **ENUM64 value (correctness):** `valLo | (valHi << 32)` -- JS `<<` is a 32-bit op, so `valHi << 32` is `valHi << 0` and the combine collapsed to `valLo | valHi` (a wrong, possibly-negative 32-bit value). Now `valLo + valHi * 0x100000000`.
> - Filed #47 for the remaining low-severity value nits (INT encoding/bits masks, ARRAY size clamp, member kflag offset) found in the same audit.
