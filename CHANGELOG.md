...
- **Known limitation / deferred (secondary):** on this PIE the `.plt.sec` stubs stay auto-named `sub_*` (e.g. `sub_5555555550E0` not `sigaction@plt`) and over-read into a shared `hlt` -- `applyPltStubNames` keys `_pltSymbolMap` by the legacy `.plt` VA while discovery lands on the `.plt.sec` VA. The new `resolveStubSymbol`/`_gotSymbolMap` plumbing makes wiring `.plt.sec` naming straightforward, but it is left OUT of this change to keep the trap-skip fix strictly scoped; deferred.
