...
- **Regression guard.** `scripts/verify-hexcore-preflight.cjs` now enumerates every `extensions/hexcore-*` whose package.json main points at `./out/` and asserts that compiled entrypoint exists, failing preflight with a message that names the missing extension and the exact CI step. This catches the whole "TS extension absent from the compile loop" class going forward, not just elixir (verified both ways: passes with out/ present, fails loudly when elixir/out/extension.js is removed).
